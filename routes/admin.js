...
router.get('/users', checkAdmin, (req, res) => {
  try {
    ...
    run("UPDATE users SET is_paid = 0 WHERE (paid_expires_at IS NULL OR paid_expires_at < datetime('now'))");
    const users = all('SELECT id, username, phone, is_paid, paid_expires_at, created_at FROM users ORDER BY id DESC');
    const now = new Date();
    const stats = {
      total: users.length,
...
router.get('/stats', checkAdmin, (req, res) => {
  try {
    ...
    const todayLogs = all("SELECT action, COUNT(*) as count FROM usage_log WHERE date(created_at) = ? GROUP BY action", [todayStr]);
    const totalLogs = all('SELECT action, COUNT(*) as count FROM usage_log GROUP BY action');
    const recentLogs = all('SELECT * FROM usage_log ORDER BY created_at DESC LIMIT 20');
